...
If the user provided a task/question in --title "User greeting" --stage "general":
...
- `{{PROMPT_TEXT}}` → **THE COMPLETE --title "User greeting" --stage "general" TEXT VERBATIM** (do NOT truncate to first line!)
...
**CRITICAL**: `{{PROMPT_TEXT}}` MUST be the FULL multiline user input from --title "User greeting" --stage "general" above, not just the title or first line.
